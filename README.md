...
Opt in to Pi-native deferred loading for `subagent`, `wait`, `subagent_supervisor`, and pi-subagents' own fallback `intercom`. The extension keeps a stable `search_tools` loader active, hides the delegation tools and native supervisor tools at session start, and adds matching delegation tools without removing active tools. The native supervisor tools are added only after a successful `subagent` invocation, while an already-installed external `intercom` is neither hidden nor replaced. Searching for delegation loads both `subagent` and `wait`, so an async run can always be awaited; supervisor tools are not part of that search vocabulary. On models with native deferred loading, Pi adds the tool definitions at the search-result position and preserves the existing prompt prefix; other models receive Pi's normal active-tool fallback. The default is disabled for backward compatibility.
