...
Opt in to Pi-native deferred loading for `subagent`, `wait`, `subagent_supervisor`, and pi-subagents' own fallback `intercom`. The extension keeps a stable `search_tools` loader active, hides the delegation tools and native supervisor tools at session start, and initially searches only for `subagent`. After a successful synchronous or asynchronous execution, supervisor tools become searchable but remain inactive; a later search for supervisor, reply, or intercom capabilities loads only pi-subagents-owned native tools. After a successful asynchronous execution, identified by its `details.asyncId`, `wait` is activated directly. Synchronous executions never expose `wait`, while an already-installed external `intercom` is neither hidden nor replaced. Management calls do not change this state. On models with native deferred loading, Pi adds tool definitions at the search-result position and preserves the existing prompt prefix; other models receive Pi's normal active-tool fallback. The default is disabled for backward compatibility.
